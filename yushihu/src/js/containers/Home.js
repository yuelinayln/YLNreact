import React,{ Component } from 'react'
import { Carousel } from 'antd-mobile'
import { Header,FastNav,Message,Title,ShopList,Nav } from '../components'

class Home extends Component {
  state = {
    fastNav: [
      {
        link: "http://localhost:8888/",
        thumb: require('./image/5.png'),
        title: "周一特色"
      },
      {
        link: "http://localhost:8888/",
        thumb: require('./image/5.png'),
        title: "周二特色"
      },
      {
        link: "http://localhost:8888/",
        thumb: require('./image/5.png'),
        title: "周三特色"
      },
      {
        link: "http://localhost:8888/",
        thumb: require('./image/5.png'),
        title: "周四特色"
      },
      {
        link: "http://localhost:8888/",
        thumb: require('./image/5.png'),
        title: "周五特色"
      },
      {
        link: "http://localhost:8888/",
        thumb: require('./image/5.png'),
        title: "周末特色"
      },
      {
        link: "http://localhost:8888/",
        thumb: require('./image/5.png'),
        title: "本周精选"
      },
      {
        link: "http://localhost:8888/",
        thumb: require('./image/5.png'),
        title: "本月活动"
      }
    ],
    shopLists: [
      {
        id: 1,
        thumb: 'http://temp.im/160x120/FF9500/000',
        label: '五折优惠',
        discount_scale: '8',
        title: '酸菜小鱼',
        caption: '本市最棒的烤鱼店',
        tag: '美食',
        distance: '2.5km',
        discount_text: '本店支持微信支付',
        discount_time: '周一到周五每天9:00-24:00'
      },{
        id: 2,
        thumb: 'http://temp.im/160x120/FF9500/000',
        label: '五折优惠',
        discount_scale: '9',
        title: '番茄小鱼',
        caption: '本市最棒的烤鱼店2',
        tag: '美食',
        distance: '12.5km',
        discount_text: '本店支持微信支付',
        discount_time: '周一到周五每天9:00-24:00'
      },{
        id: 3,
        thumb: 'http://temp.im/160x120/FF9500/000',
        label: '五折优惠',
        discount_scale: '8.8',
        title: '麻椒小鱼',
        caption: '本市最棒的烤鱼店2',
        tag: '美食',
        distance: '12.5km',
        discount_text: '本店支持微信支付',
        discount_time: '周一到周五每天9:00-24:00'
      }
    ],
    messages: [
      {
        link: 'http://localhost:8888/',
        desc: '重庆,热情与美食的天堂'
      },{
        link: 'http://localhost:8888/',
        desc: '花花椒椒海洋中的热辣山城'
      },{
        link: 'http://localhost:8888/',
        desc: '荡漾着味道巴蜀的麻辣鲜香'
      }

    ]
  }

  render() {
    const settings = {
      dots: true,
      autoplay: true,
      autoplaySpeed: 3000,
      infinite: true,
      mode: 'banner',
      initialSlide: 0,
      afterChange: this.slideTo
    }
    const { fastNav,messages,shopLists } = this.state

    return (
      <div>
        <Header />
        <Carousel {...settings}>
          <div className="HomeItem">
            <img src={require('./image/1.png')} className="homeImg" alt=""/>
          </div>
          <div className="HomeItem">
            <img src={require('./image/2.png')} className="homeImg" alt=""/>
          </div>
          <div className="HomeItem">
            <img src={require('./image/3.png')} className="homeImg" alt=""/>
          </div>
          <div className="HomeItem">
            <img src={require('./image/4.png')} className="homeImg" alt=""/>
          </div>
        </Carousel>
        <div className="bg-white hor pt30">
            {fastNav.map((item,index)=>
                <FastNav key={index} data={item} />
            )}
        </div>
        <Message data={messages} />
        <Title text="精品好店" linkName="查看全部" path="/shop" />
        <div className="plr20 bg-white">
            {shopLists.map(item=>
              <ShopList key={item.id} data={item} />
            )}
        </div>
        <Nav />
      </div>
    )
  }
}

export default Home